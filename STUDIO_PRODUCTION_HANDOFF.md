...
| --- | --- | --- |
| Image generation | `IMAGE_PROVIDER=stability`, `STABILITY_API_KEY`, `STABILITY_IMAGE_MODEL=ultra` | Image generation and editing report unavailable when the Stability key is absent, invalid, out of credits, rate-limited, or rejected by moderation. Use `IMAGE_PROVIDER=fal` only when a funded fal.ai account is intentionally selected. |
| Captions | `CAPTION_PROVIDER=openrouter`, `OPENROUTER_API_KEY`, `OPENROUTER_MODELS` | OpenRouter tries the ordered model list and switches models on token-limit, rate-limit, model-unavailable, and transient provider failures. If OpenRouter is not selected, Gemini and optional Ollama remain available. `GEMINI_API_KEY` remains a compatibility fallback. |
...
When `CAPTION_PROVIDER=openrouter`, the caption service sends the same prompt to an ordered, comma-separated `OPENROUTER_MODELS` list, with a bounded number of attempts controlled by `OPENROUTER_MAX_ATTEMPTS`. It changes models only for retryable provider conditions such as token limits, rate limits, unavailable models, transient 5xx errors, empty responses, and request-size failures; it does not retry invalid credentials or other non-retryable errors. Model identifiers and provider response bodies are never logged with API keys. The Gemini/Veo adapter submits a long-running operation, polls until completion or timeout, then downloads the returned video. The automated test suite mocks each of these phases. A passing test proves the adapter contract and lifecycle; it does not prove that a particular Google account has Veo access. Caption generation uses `GEMINI_CAPTION_API_KEY`, while Veo uses `GEMINI_VIDEO_API_KEY`; if either is absent, that service falls back to `GEMINI_API_KEY`.
