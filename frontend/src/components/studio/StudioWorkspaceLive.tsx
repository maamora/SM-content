...
function workspaceCanvasNodes(products: Product[], posts: Post[]): CanvasNode[] {
    const product = products[0]; const post = posts[0];
    return [
        { id: "workspace-prompt", kind: "prompt", label: "PROMPT", title: product ? `Build from ${product.name}` : "Start with a direction", meta: product ? "Source attached" : "No source yet", detail: product ? "The live source material is attached to the next creative branch." : "Add a product or open Studio to turn the next thought into a branch.", x: 17, y: 34, status: product ? "live" : "draft" },
        { id: "workspace-product", kind: "image", label: "SOURCE", title: product?.name ?? "Product reference", meta: product ? `${product.status} · product` : "Waiting for input", detail: "The source card is the anchor for every generated direction.", imageUrl: product?.imageUrl ?? undefined, x: 43, y: 23, status: product?.status === "APPROVED" ? "ready" : "draft" },
        { id: "workspace-post", kind: "image", label: "OUTPUT", title: post?.productName ?? "Next output", meta: post ? `${post.status} · ${post.format}` : "No post yet", detail: post ? "A real post record connected to the live workspace." : "Generated posts will appear here after the first studio run.", imageUrl: post?.imageUrl ?? undefined, x: 69, y: 35, status: post ? "ready" : "draft" },
        { id: "workspace-video", kind: "video", label: "BRANCH", title: "Motion branch", meta: "Ready when you are", detail: "A future-facing branch for motion outputs, kept visible without pretending it is persisted yet.", x: 43, y: 72, status: "draft" },
        { id: "workspace-palette", kind: "palette", label: "BRAND", title: "Brand signal", meta: "Shared context", detail: "Brand settings keep the visual family coherent across every branch.", x: 83, y: 69, status: "live" },
    ];
}

function WorkspaceCanvas({ products, posts }: { products: Product[]; posts: Post[] }) {
    return <section className="studio-workspace-panel studio-workspace-panel--canvas"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">CREATIVE THREAD</span><h2>Keep the signal attached.</h2></div><span className="studio-chip studio-chip--lime">LIVE CANVAS</span></div><CreativeCanvas nodes={workspaceCanvasNodes(products, posts)} title="Workspace / live thread" eyebrow="STUDIO / CANVAS" compact /></section>;
}

function Overview({ products, posts, refresh }: { products: Product[]; posts: Post[]; refresh: () => void }) {
    const recent = posts.slice(0, 5);
    return <><WorkspaceCanvas products={products} posts={posts} /><Stats products={products} posts={posts} /><div className="studio-workspace-grid"><section className="studio-workspace-panel studio-workspace-panel--wide"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">LIVE BOARD</span><h2>Real workspace movement</h2></div><button className="studio-text-button" onClick={refresh}><RefreshCw size={14} /> Refresh</button></div>{recent.length ? <div className="studio-data-stack">{recent.map((post) => <Link className="studio-data-row" key={post.id} href="/dashboard/posts"><span className="studio-status-dot" /><div><strong>{post.productName}</strong><small>{post.status} · {post.format}</small></div><span className="studio-data-value">{post.createdAt ? new Date(post.createdAt).toLocaleDateString() : "New"}</span><ChevronRight size={14} /></Link>)}</div> : <Notice title="Your live board is waiting." detail="Create a product, then open Studio to generate the first real post." action={<Link className="studio-button studio-button--dark" href="/dashboard/products"><Plus size={14} /> Add product</Link>} />}</section><section className="studio-workspace-panel studio-workspace-panel--accent"><span className="studio-kicker studio-kicker--dark">NEXT MOVE</span><h2>Turn one approved product into a full campaign.</h2><p>Use the real product and template APIs to generate, caption, approve, and export.</p><Link href="/dashboard/studio" className="studio-button studio-button--dark">Open studio <ArrowUpRight size={15} /></Link></section></div></>;
}
...
function Surface({ mode, products, posts, refresh }: { mode: WorkspaceMode; products: Product[]; posts: Post[]; refresh: () => void }) {
    ...
    if (mode === "studio") return <><WorkspaceCanvas products={products} posts={posts} /><section className="studio-workspace-panel studio-workspace-panel--wide"><CreativeStudio products={products} onPostChange={refresh} /></section></>;
...
function AdminSurface({ mode, products, posts, templates }: { mode: AdminMode; products: Product[]; posts: Post[]; templates: Template[] }) {
    ...
    if (mode === "dashboard" || mode === "analytics") return <><section className="studio-workspace-panel studio-workspace-panel--canvas"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">CONTROL THREAD</span><h2>{mode === "analytics" ? "Read the motion." : "Keep the system visible."}</h2></div><span className="studio-chip studio-chip--lime">LIVE API</span></div><CreativeCanvas nodes={workspaceCanvasNodes(products, posts)} title="Admin / system thread" eyebrow="CONTROL ROOM / LIVE" compact /></section><section className="studio-workspace-panel"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">BACKEND SIGNAL</span><h2>{mode === "analytics" ? "Observed workspace motion" : "System overview"}</h2></div></div><div className="studio-stat-grid studio-stat-grid--admin"><div><span>PRODUCTS</span><strong>{products.length}</strong><small>Admin-visible source material</small></div><div><span>POSTS</span><strong>{posts.length}</strong><small>Generated content records</small></div><div><span>TEMPLATES</span><strong>{templates.length}</strong><small>Live API response</small></div><div><span>API HEALTH</span><strong>LIVE</strong><small>Authenticated routes responding</small></div></div></section></>;
