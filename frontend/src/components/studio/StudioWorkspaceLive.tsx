...
const workspaceData = {
    dashboard: ["Overview", "The work, in motion.", "A live read on the source, direction, post, and delivery thread.", LayoutDashboard],
...
function Stats({ products, posts }: { products: Product[]; posts: Post[] }) {
    ...
    return <div className="studio-stat-grid studio-overview-stats"><div><span>PRODUCTS</span><strong>{products.length}</strong><small>{approved} approved</small></div><div><span>POSTS</span><strong>{posts.length}</strong><small>{ready} ready to ship</small></div><div><span>READY</span><strong>{ready}</strong><small>{ready ? "Ready to review" : "Awaiting first post"}</small></div><div><span>RECENT ACTIVITY</span><strong>{posts.length ? "LIVE" : "—"}</strong><small>{last}</small></div></div>;
}
...
function Overview({ products, posts, refresh }: { products: Product[]; posts: Post[]; refresh: () => void }) {
    ...
    return <div className="studio-worktable studio-worktable--overview"><header className="studio-worktable__masthead"><div><span className="studio-kicker studio-kicker--dark">WORKSPACE / 01</span><h1>The work, held in one frame.</h1><p>Read the source, the visual direction, the post, and the hand-off without leaving the creative thread.</p></div><div className="studio-worktable__masthead-actions"><button className="studio-text-button" onClick={refresh}><RefreshCw size={14} /> Refresh records</button><Link href="/dashboard/studio" className="studio-button studio-button--dark"><Plus size={15} /> New direction</Link></div></header><FadeContent duration={260} distance={8} threshold={0.08} className="studio-quiet-reveal"><Stats products={products} posts={posts} /></FadeContent><FadeContent duration={300} delay={45} distance={10} threshold={0.08} className="studio-quiet-reveal"><div className="studio-worktable__body"><section className="studio-overview-runway"><div className="studio-overview-runway__heading"><div><span className="studio-kicker">CREATIVE THREAD</span><h2>From material to moment.</h2></div><span>{sourceProducts.length ? `${sourceProducts.length} source record${sourceProducts.length === 1 ? "" : "s"}` : "No source yet"}</span></div>{sourceProducts.length ? <div className="studio-overview-runway__sequence"><div className="studio-overview-runway__group"><span>01 / SOURCE</span><div className="studio-overview-flow__assets">{sourceProducts.map((product) => <Link href="/dashboard/products" key={product.id} className="studio-overview-flow__asset">{product.imageUrl ? <img src={product.imageUrl} alt="" /> : <span />}<strong>{product.name}</strong><small>{product.status}</small></Link>)}</div></div><div className="studio-overview-runway__step">→</div><Link href="/dashboard/studio" className="studio-overview-runway__card"><span>02 / DIRECTION</span><strong>{latestPost?.format || "New post"}</strong><small>{latestPost ? "Saved direction ready to review" : "Choose a format and compose"}</small></Link><div className="studio-overview-runway__step">→</div><Link href="/dashboard/posts" className="studio-overview-runway__card studio-overview-runway__card--post"><span>03 / POST</span>{latestPost?.imageUrl ? <img src={latestPost.imageUrl} alt="" /> : <b>Draft</b>}<strong>{latestPost?.productName || "No post saved"}</strong><small>{latestPost?.status || "Awaiting composition"}</small></Link><div className="studio-overview-runway__step">→</div><Link href="/dashboard/social" className="studio-overview-runway__card"><span>04 / DELIVERY</span><strong>{latestPost?.status === "APPROVED" ? "Ready" : "Review"}</strong><small>{latestPost?.status === "APPROVED" ? "Choose a channel and time" : "Validate the post first"}</small></Link></div> : <Notice title="Your creative thread is waiting." detail="Add a real product reference, then use Studio to create the first post direction." action={<Link className="studio-button studio-button--lime" href="/dashboard/products"><Plus size={14} /> Add product</Link>} />}</section><aside className="studio-overview-next"><span className="studio-kicker studio-kicker--dark">NEXT MOVE</span><div><span className="studio-overview-next__eyebrow">{sourceProducts.length ? "DIRECTION REVIEW" : "SOURCE SETUP"}</span><h2>{sourceProducts.length ? "Turn this source into a considered post." : "Start the first creative thread."}</h2><p>{sourceProducts.length ? "Set the direction in Studio, then move into captions, review, and delivery." : "Add source material to unlock local composition and multilingual captions."}</p></div><Link href={nextHref} className="studio-overview-next__link">{nextLabel} <ArrowUpRight size={16} /></Link></aside></div></FadeContent><FadeContent duration={260} delay={80} distance={8} threshold={0.08} className="studio-quiet-reveal"><WorkspaceReadiness products={products} posts={posts} /></FadeContent></div>;
}
...
function ProductsSurface({ products, refresh }: { products: Product[]; refresh: () => void }) {
    return <div className="studio-worktable studio-worktable--source-library"><header className="studio-worktable__masthead"><div><span className="studio-kicker studio-kicker--dark">SOURCE LIBRARY / 02</span><h1>Keep the material close.</h1><p>Every product reference stays visible, intentional, and ready for the next local composition.</p></div><div className="studio-worktable__masthead-actions"><span className="studio-worktable__count">{products.length} source record{products.length === 1 ? "" : "s"}</span><Link href="/dashboard/studio" className="studio-button studio-button--dark">Open Studio <ArrowUpRight size={15} /></Link></div></header><div className="studio-source-library"><section className="studio-source-library__catalog"><FadeContent duration={280} distance={8} threshold={0.1} className="studio-quiet-reveal"><div className="studio-source-library__section-head"><span>PRODUCT CONTACT SHEET</span><p>Use the library as the source of truth—review, remove, or move into Studio.</p></div><ProductList products={products} onProductDeleted={refresh} /></FadeContent></section><FadeContent duration={300} delay={35} distance={8} threshold={0.1} className="studio-quiet-reveal"><BrandLibraryPanel /></FadeContent><section className="studio-source-library__upload"><FadeContent duration={320} delay={65} distance={8} threshold={0.1} className="studio-quiet-reveal"><div><span className="studio-kicker studio-kicker--dark">ADD SOURCE</span><h2>Bring in the next material.</h2><p>Upload the product reference that will anchor the next direction.</p></div><ProductForm onCreated={refresh} /><TestingCatalog products={products} onCreated={refresh} /></FadeContent></section></div></div>;
}

function BrandWorktable() {
    return <div className="studio-worktable studio-worktable--brand"><header className="studio-worktable__masthead"><div><span className="studio-kicker studio-kicker--dark">BRAND RULES / 03</span><h1>Make the system recognizably yours.</h1><p>Keep the identity accessible at the moment it supports a post—not imposed on every output.</p></div><div className="studio-worktable__masthead-actions"><Link href="/dashboard/studio" className="studio-button studio-button--dark">Open Studio <ArrowUpRight size={15} /></Link></div></header><BrandSurface /></div>;
}
...
export function WorkspacePage({ mode }: { mode: WorkspaceMode }) {
    ...
    return <main className={`studio-app studio-app--${mode} ${isStudioRoute ? "studio-app--studio" : ""}`}><WorkspaceSidebar active={mode} /><div className="studio-workspace-main">{!isStudioRoute && !isWorktableRoute && <header className="studio-workspace-topbar"><div><span className="studio-kicker studio-kicker--dark">WORKSPACE / {label.toUpperCase()}</span><h1>{title}</h1><p>{description}</p></div><div className="studio-workspace-actions"><button className="studio-icon-button" aria-label={`Focus ${label.toLowerCase()} search`} onClick={() => searchRef.current?.focus()}><Search size={17} /></button><Link href="/dashboard/studio" className="studio-button studio-button--dark"><Plus size={15} /> New direction</Link></div></header>}<section className={`studio-workspace-content ${isWorktableRoute ? "studio-workspace-content--worktable" : ""}`}>{!isStudioRoute && !isWorktableRoute && <div className="studio-command-row"><div className="studio-route-title"><Icon size={20} /><span>{label}</span></div><label className="studio-search"><Search size={15} /><input ref={searchRef} placeholder={`Search ${label.toLowerCase()}`} aria-label={`Search ${label.toLowerCase()}`} /></label></div>}{error && <div className="studio-form-error"><strong>Live data unavailable.</strong> {error} <button onClick={() => void reload()}>Retry</button></div>}{loading ? <div className="studio-loading"><Loader2 className="studio-spin" size={18} /> Loading live workspace data…</div> : <Surface mode={mode} products={products} posts={posts} refresh={() => void reload({ background: true })} onStudioPostChange={() => void reload({ background: true })} />}</section></div></main>;
}
