...
function Overview({ products, posts, refresh }: { products: Product[]; posts: Post[]; refresh: () => void }) {
    ...
    return <div className="studio-overview"><Stats products={products} posts={posts} /><div className="studio-overview-board"><section className="studio-overview-thread"><div className="studio-panel-heading"><div><span className="studio-kicker">CREATIVE THREAD</span><h2>Source to delivery</h2></div><button className="studio-text-button" onClick={refresh}><RefreshCw size={14} /> Refresh</button></div>{sourceProducts.length ? <div className="studio-overview-flow"><div className="studio-overview-flow__group"><span>PRODUCT</span><div className="studio-overview-flow__assets">{sourceProducts.map((product) => <Link href="/dashboard/products" key={product.id} className="studio-overview-flow__asset">{product.imageUrl ? <img src={product.imageUrl} alt="" /> : <span /> }<strong>{product.name}</strong><small>{product.status}</small></Link>)}</div></div><div className="studio-overview-flow__arrow">→</div><div className="studio-overview-flow__group"><span>DIRECTION</span><Link href="/dashboard/studio" className="studio-overview-flow__asset studio-overview-flow__direction"><b>{latestPost?.format || "New post"}</b><small>{latestPost ? "Saved direction" : "Ready to compose"}</small></Link></div><div className="studio-overview-flow__arrow">→</div><div className="studio-overview-flow__group"><span>POST</span><Link href="/dashboard/posts" className="studio-overview-flow__asset">{latestPost?.imageUrl ? <img src={latestPost.imageUrl} alt="" /> : <span /> }<strong>{latestPost?.productName || "No post yet"}</strong><small>{latestPost?.status || "Draft"}</small></Link></div><div className="studio-overview-flow__arrow">→</div><div className="studio-overview-flow__group"><span>DELIVERY</span><Link href="/dashboard/social" className="studio-overview-flow__asset studio-overview-flow__direction"><b>{latestPost?.status === "APPROVED" ? "Ready" : "Review"}</b><small>Open Social</small></Link></div></div> : <Notice title="Your creative thread is waiting." detail="Add a real product reference, then use Studio to create the first post direction." action={<Link className="studio-button studio-button--lime" href="/dashboard/products"><Plus size={14} /> Add product</Link>} />}</section><section className="studio-overview-next"><span className="studio-kicker studio-kicker--dark">NEXT MOVE</span><div><span className="studio-overview-next__eyebrow">{sourceProducts.length ? "DIRECTION REVIEW" : "SOURCE SETUP"}</span><h2>{sourceProducts.length ? "Turn this source into a considered post." : "Start the first creative thread."}</h2><p>{sourceProducts.length ? "Choose the product, set the direction, then move through captions, review, and delivery." : "Add source material to unlock local composition and multilingual captions."}</p></div><Link href={nextHref} className="studio-overview-next__link">{nextLabel} <ArrowUpRight size={16} /></Link></section></div><WorkspaceReadiness products={products} posts={posts} /></div>;
}
...
function Surface({ mode, products, posts, refresh, onStudioPostChange }: { mode: WorkspaceMode; products: Product[]; posts: Post[]; refresh: () => void; onStudioPostChange: () => void }) {
    ...
    if (mode === "products") return <div className="studio-source-library"><section className="studio-source-library__catalog"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">SOURCE LIBRARY / 02</span><h2>Product references</h2></div><span className="studio-chip">{products.length} total</span></div><ProductList products={products} onProductDeleted={refresh} /></section><BrandLibraryPanel /><section className="studio-source-library__upload"><div><span className="studio-kicker studio-kicker--dark">ADD PRODUCT REFERENCE</span><h2>Bring in the next source.</h2><p>Upload real product material that powers the local composition workflow.</p></div><ProductForm onCreated={refresh} /><TestingCatalog products={products} onCreated={refresh} /></section></div>;
...
export function WorkspacePage({ mode }: { mode: WorkspaceMode }) {
    ...
    return <main className={`studio-app ${isStudioRoute ? "studio-app--studio" : ""}`}><WorkspaceSidebar active={mode} /><div className="studio-workspace-main">{!isStudioRoute && <header className="studio-workspace-topbar"><div><span className="studio-kicker studio-kicker--dark">WORKSPACE / {label.toUpperCase()}</span><h1>{title}</h1><p>{description}</p></div><div className="studio-workspace-actions"><button className="studio-icon-button" aria-label={`Focus ${label.toLowerCase()} search`} onClick={() => searchRef.current?.focus()}><Search size={17} /></button><Link href="/dashboard/studio" className="studio-button studio-button--dark"><Plus size={15} /> New direction</Link></div></header>}<section className="studio-workspace-content">{!isStudioRoute && <div className="studio-command-row"><div className="studio-route-title"><Icon size={20} /><span>{label}</span></div><label className="studio-search"><Search size={15} /><input ref={searchRef} placeholder={`Search ${label.toLowerCase()}`} aria-label={`Search ${label.toLowerCase()}`} /></label></div>}{error && <div className="studio-form-error"><strong>Live data unavailable.</strong> {error} <button onClick={() => void reload()}>Retry</button></div>}{loading ? <div className="studio-loading"><Loader2 className="studio-spin" size={18} /> Loading live workspace data…</div> : <Surface mode={mode} products={products} posts={posts} refresh={() => void reload({ background: true })} onStudioPostChange={() => void reload({ background: true })} />}</section></div></main>;
}
