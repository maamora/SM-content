...
function NotificationsSurface({ products, posts }: { products: Product[]; posts: Post[] }) {
    ...
    return <section className="studio-workspace-panel"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">LIVE SIGNALS</span><h2>Notifications from real records</h2></div><span className="studio-chip">{notices.length} open</span></div>{notices.length ? <div className="studio-data-stack">{notices.map((notice) => <Link className="studio-data-row" key={notice.id} href={notice.href}><span className="studio-status-dot studio-status-dot--lime" /><div><strong>{notice.title}</strong><small>{notice.detail}</small></div><ChevronRight size={14} /></Link>)}</div> : <Notice title="Nothing needs your attention." detail="Approval and draft events will appear here as your workspace changes." />}</section>;
}

function SocialSurface({ posts }: { posts: Post[] }) {
    return <section className="studio-workspace-panel"><div className="studio-panel-heading"><div><span className="studio-kicker studio-kicker--dark">PUBLISHING READINESS</span><h2>Social channels</h2></div><span className="studio-chip">{posts.length} exportable posts</span></div><div className="studio-live-columns"><div><h3>Local workflow ready</h3><p>Posts can be approved, downloaded, and handed to any channel using the export flow. No social account tokens are stored until an explicit provider integration is configured.</p><Link className="studio-button studio-button--dark" href="/dashboard/posts"><Download size={14} /> Open export queue</Link></div><div className="studio-unavailable"><span className="studio-kicker studio-kicker--dark">PROVIDER STATUS</span><h3>OAuth publishing is not configured</h3><p>Instagram, TikTok, and LinkedIn publishing require provider credentials and review. The UI will not claim a post was published without a confirmed provider response.</p><Link className="studio-button studio-button--outline" href="/dashboard/settings"><Settings2 size={14} /> Review settings</Link></div></div></section>;
}
...
function SettingsSurface() {
    ...
    const capabilityRows: [string, boolean][] = [["Caption generation", capabilities.captionGeneration], ["Image generation", capabilities.imageGeneration], ["Cloud storage", capabilities.cloudStorage], ["Local storage", capabilities.localStorage], ["Social publishing", capabilities.socialPublishing], ["Email delivery", capabilities.emailDelivery]];
