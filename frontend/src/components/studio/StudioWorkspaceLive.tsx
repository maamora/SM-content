...
function ProductsSurface({ products, refresh }: { products: Product[]; refresh: () => void }) {
    return <div className="studio-worktable studio-worktable--source-library"><header className="studio-worktable__masthead"><div><span className="studio-kicker studio-kicker--dark">SOURCE LIBRARY / 02</span><h1>Keep the material close.</h1><p>Every product reference stays visible, intentional, and ready for the next local composition.</p></div><div className="studio-worktable__masthead-actions"><span className="studio-worktable__count">{products.length} source record{products.length === 1 ? "" : "s"}</span><Link href="/dashboard/studio" className="studio-button studio-button--dark">Open Studio <ArrowUpRight size={15} /></Link></div></header><div className="studio-source-library"><section className="studio-source-library__catalog"><div className="studio-source-library__section-head"><span>PRODUCT CONTACT SHEET</span><p>Use the library as the source of truth—review, remove, or move into Studio.</p></div><ProductList products={products} onProductDeleted={refresh} /></section><BrandLibraryPanel /><section className="studio-source-library__upload"><div><span className="studio-kicker studio-kicker--dark">ADD SOURCE</span><h2>Bring in the next material.</h2><p>Upload the product reference that will anchor the next direction.</p></div><ProductForm onCreated={refresh} /><TestingCatalog products={products} onCreated={refresh} /></section></div></div>;
}
