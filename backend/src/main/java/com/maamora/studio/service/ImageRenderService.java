...
public class ImageRenderService {
    ...
    public RenderedVisual render(Template template, Product product,
        ...
        String heroImageOverride = null;
        GenerationMode mode = GenerationMode.TEMPLATE_COMPOSED;
        String recoveryMessage = "Created locally from the selected SVG template. No AI provider or image-generation API was used.";

        if (imageGenerationProvider.isConfigured()) {
            try {
                String aspectRatio = isSquare ? "1:1" : "9:16";
                ...
                List<String> references = product.getImageUrl() != null && !product.getImageUrl().isBlank()
                        ? List.of(product.getImageUrl())
                        : List.of();
                byte[] aiPng = imageGenerationProvider.generateImage(prompt, aspectRatio, references);
                heroImageOverride = "data:image/png;base64," + Base64.getEncoder().encodeToString(aiPng);
                mode = GenerationMode.AI_GENERATED;
                recoveryMessage = "Generated by " + imageGenerationProvider.activeProvider() + ", composed into the selected template.";
            } catch (RuntimeException aiFailure) {
                log.warn("AI image generation via {} failed; falling back to the local SVG template: {}",
                        imageGenerationProvider.activeProvider(), aiFailure.getMessage());
                recoveryMessage = "AI image generation failed (" + compactMessage(aiFailure.getMessage())
                        + "); created locally from the selected SVG template instead.";
            }
        }
        ...
        byte[] png = svgTemplateRenderer.render(product, width, height, badgeText, promoText, accentColor, mood,
                ...
                productFocus, textAlignment, heroImageOverride);
        return new RenderedVisual(png, mode, recoveryMessage);
    }
