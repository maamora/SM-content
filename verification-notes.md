...
- A fresh automated recheck passed: PostgreSQL readiness, login, `/api/products`, `/api/products/pending`, `/api/posts`, `/api/brand`, `/api/templates`, CORS preflight, all representative Next.js routes, TypeScript, and lint.
- `/api/auth/me` is not implemented by the current Spring Boot `AuthController`; the frontend does not call it because it validates the signed session token client-side and the backend enforces authorization on every protected resource endpoint. It should be added later if server-side session introspection is required.
